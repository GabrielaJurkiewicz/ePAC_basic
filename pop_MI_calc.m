...
%     MI:
%     Calculates the Modulation Index - measure of
%     phase-amplitude coupling for given data.
% 
%     Usage:
%       >> pop_MI_calc(EEG);          % pop_up window
%
%       >> MI_calc(EEG,chan_fP,chan_fA,epochs,fPstart,fPend,fPstep,fPfiltband,fAstart,fAend,fAstep,fAfiltband,...
%                    dirOut,type,nbBins,Nboot,pMI,artifacts,plotWithMask,plotWithoutMask);
% 
%     Input:
...
%
%       - Percentile of surrogate data PAC measure distribution (pMI):
%         [1-100] threshold for detecting statistically significant values of MI. It is expressed as percentile of 
%         distribution of maximal MI values from each comodulogram for surrogate data.
% 
...
%               * pval - array (fA x fP) with percentiles of surrogate data MI distibution assigned to each value in comodulogram
%               * comodulogramStat - array (fA x fP), comodulogram with statistical mask on

function [LASTCOM] = pop_MI_calc(EEG , varargin)
    ...
    defaults.fAstart    = 20;
    defaults.fAend      = floor(EEG.srate/2/1.15);
    defaults.fAstep     = 5;
    ...
    if nargin < 18
        ...
        title_string = 'Detects phase-amplitude coupling by calculating Modulation Index -- pop_MI_calc()';
        ...
        uilist = { ...
         ...
         { 'style', 'edit', 'string', num2str(defaults.pMI), 'tag', 'pMI'}...
         };
     
        [~, ~, err params] = inputgui( 'geometry', geometry, 'geomvert', geomvert, 'uilist', uilist, 'helpcom', 'pophelp(''pop_MI_calc'');', 'title' , title_string);
    ...
    elseif nargin == 20
        % MI_calc(EEG,chan_fP,chan_fA,epochs,fPstart,fPend,fPstep,fPfiltband,fAstart,fAend,fAstep,fAfiltband,dirOut,type,nbBins,Nboot,pMI,artifacts,plotWithMask,plotWithoutMask);
        ...
        try
            ...
            params.pMI        = varargin{16};
            params.artifacts  = varargin{17};
            params.plotWithMask = varargin{18};
            params.plotWithoutMask = varargin{19};
    ...
    try
        ...
        MI_calc(EEG,params.chan_fP,params.chan_fA,params.epochs,params.fPstart,params.fPend,params.fPstep,params.fPfiltband,params.fAstart,params.fAend,...
                       params.fAstep,params.fAfiltband,params.dirOut,params.type,params.nbBins,params.Nboot,params.pMI,params.artifacts,params.plotWithMask,params.plotWithoutMask);
             
        tmpstring = '[LASTCOM] = pop_MI_calc(EEG';
